...
function drawTextCenteredOn(text, fontSize, color, x, y) {
    ctx.fillStyle = color;
...
function drawSelection(position) {
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(position.col * tileSize + tileSize / 2, position.row * tileSize + tileSize / 2, tileSize / 2 + 2, // Adjust the radius to your liking
    0, Math.PI * 2);
    ctx.stroke();
}
